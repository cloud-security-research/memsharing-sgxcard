...
struct sgx5_map_struct stored_maps[MAX_MAPS_NR] = {{0,},};
...
static ssize_t read_map2(struct kobject *kobj, struct kobj_attribute *attr,
                      char *buf)
{
        return sprintf(buf,"0x%lx 0x%lx 0x%lx\n",stored_maps[2].remote_phys_addr,stored_maps[2].len,stored_maps[2].local_phys_addr);
}


static ssize_t write_map2(struct kobject *kobj, struct kobj_attribute *attr,
                     const char *buf, size_t count)
{
        sscanf(buf, "0x%lx 0x%lx", &stored_maps[2].remote_phys_addr, &stored_maps[2].len);
        printk("somnath: phys_addr=0x%lx len=0x%lx\n",stored_maps[2].remote_phys_addr,stored_maps[2].len);

        if ((stored_maps[2].remote_phys_addr == 0) || (stored_maps[2].len == 0)) {
                if (stored_maps[2].local_phys_addr != 0) {
                        sgx5_unmap(&stored_maps[2].local_phys_addr);
                        printk("somnath: unmap success remote_phys_addr=0x%lx len=0x%lx\n",stored_maps[2].remote_phys_addr,stored_maps[2].len);
                }
                stored_maps[2].len = 0;
                stored_maps[2].remote_phys_addr = 0;
        } else {   
                if (sgx5_map(stored_maps[2].remote_phys_addr,stored_maps[2].len,&stored_maps[2].local_phys_addr) != 0) {
                        printk("somnath: mapping fail remote_phys_addr=0x%lx len=0x%lx\n",stored_maps[2].remote_phys_addr,stored_maps[2].len);
                        stored_maps[2].remote_phys_addr = 0;
                        stored_maps[2].len = 0;
                        return -1;
...
static struct kobj_attribute map2_attribute =__ATTR(map2, 0660, read_map2, write_map2);

static int __init sgx5_init (void)
{
        int error = 0;
	char mapper_name[32] = {0,};
	long nodeid = -1;
	somnath_xdev = (struct plx_device *)vca_get_xdev_pointer(0,0); 		
	printk("somnath:vca getxdevpointer %p hostname %s\n",vca_get_xdev_pointer,utsname()->nodename);
	printk("somnath: xdev %p aper.va %p aper.pa 0x%llx\n",somnath_xdev, somnath_xdev->aper.va, somnath_xdev->aper.pa);
	kstrtol(&utsname()->nodename[strlen(utsname()->nodename)-2],10,&nodeid);
	snprintf(mapper_name,32,"sgx5_mapper_%d",(int)(((nodeid/10)*3) + (nodeid%10)));
        sgx5_kobject = kobject_create_and_add(mapper_name,kernel_kobj);
        if(!sgx5_kobject)
                return -ENOMEM;

        error = sysfs_create_file(sgx5_kobject, &map0_attribute.attr);
        if (error) {
                pr_debug("somnath: failed to create the map 0 file in /sys/kernel/sgx5_mapper \n");
        }


        error = sysfs_create_file(sgx5_kobject, &map1_attribute.attr);
        if (error) {
                pr_debug("somnath: failed to create the map 1 file in /sys/kernel/sgx5_mapper \n");
        }

        error = sysfs_create_file(sgx5_kobject, &map2_attribute.attr);
        if (error) {
                pr_debug("somnath: failed to create the map 2 file in /sys/kernel/sgx5_mapper \n");
        }
...
static void __exit sgx5_exit (void)
{
        pr_debug ("somnath: Module un initialized successfully \n");
        kobject_put(sgx5_kobject);
}


module_init(sgx5_init);
module_exit(sgx5_exit);
MODULE_LICENSE("GPL");
