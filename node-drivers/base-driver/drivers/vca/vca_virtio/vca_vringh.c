...
#include "include/vca_vringh.h"
#include "include/vca_virtio_ring.h"
#include "uapi/vca_virtio_config.h"
 ...
 * yourself!
 */
int vca_vringh_init_user(struct vringh *vrh, u64 features,
...
EXPORT_SYMBOL(vca_vringh_init_user);
 ...
 */
int vca_vringh_getdesc_user(struct vringh *vrh,
...
EXPORT_SYMBOL(vca_vringh_getdesc_user);
 ...
 */
ssize_t vca_vringh_iov_pull_user(struct vringh_iov *riov, void *dst, size_t len)
{
	return vringh_iov_xfer((struct vringh_kiov *)riov,
			       dst, len, xfer_from_user);
}
EXPORT_SYMBOL(vca_vringh_iov_pull_user);
 ...
 * Returns the bytes copied <= len or a negative errno.
 */
ssize_t vca_vringh_iov_push_user(struct vringh_iov *wiov,
			     const void *src, size_t len)
{
	return vringh_iov_xfer((struct vringh_kiov *)wiov,
			       (void *)src, len, xfer_to_user);
}
EXPORT_SYMBOL(vca_vringh_iov_push_user);
 ...
 */
void vca_vringh_abandon_user(struct vringh *vrh, unsigned int num)
{
	/* We only update vring_avail_event(vr) when we want to be notified,
	 * so we haven't changed that yet. */
	vrh->last_avail_idx -= num;
}
EXPORT_SYMBOL(vca_vringh_abandon_user);
 ...
 * to this function.
 */
int vca_vringh_complete_user(struct vringh *vrh, u16 head, u32 len)
{
...
EXPORT_SYMBOL(vca_vringh_complete_user);
 ...
 * to this function.
 */
int vca_vringh_complete_multi_user(struct vringh *vrh,
			       const struct vring_used_elem used[],
			       unsigned num_used)
{
...
EXPORT_SYMBOL(vca_vringh_complete_multi_user);
 ...
 */
bool vca_vringh_notify_enable_user(struct vringh *vrh)
{
	return __vringh_notify_enable(vrh, getu16_user, putu16_user);
}
EXPORT_SYMBOL(vca_vringh_notify_enable_user);
 ...
 */
void vca_vringh_notify_disable_user(struct vringh *vrh)
{
	__vringh_notify_disable(vrh, putu16_user);
}
EXPORT_SYMBOL(vca_vringh_notify_disable_user);
 ...
 */
int vca_vringh_need_notify_user(struct vringh *vrh)
{
	return __vringh_need_notify(vrh, getu16_user);
}
EXPORT_SYMBOL(vca_vringh_need_notify_user);
 ...
 */
int vca_vringh_init_kern(struct vringh *vrh, u64 features,
		     unsigned int num, bool weak_barriers,
...
EXPORT_SYMBOL(vca_vringh_init_kern);
 ...
 * Note that you may need to clean up riov and wiov, even on error!
 */
int vca_vringh_getdesc_kern(struct vringh *vrh,
			struct vringh_kiov *riov,
			struct vringh_kiov *wiov,
			u16 *head,
			gfp_t gfp)
...
EXPORT_SYMBOL(vca_vringh_getdesc_kern);
 ...
 * Returns the bytes copied <= len or a negative errno.
 */
ssize_t vca_vringh_iov_pull_kern(struct vringh_kiov *riov, void *dst, size_t len)
{
	return vringh_iov_xfer(riov, dst, len, xfer_kern);
}
EXPORT_SYMBOL(vca_vringh_iov_pull_kern);
 ...
 * Returns the bytes copied <= len or a negative errno.
 */
ssize_t vca_vringh_iov_push_kern(struct vringh_kiov *wiov,
			     const void *src, size_t len)
{
	return vringh_iov_xfer(wiov, (void *)src, len, xfer_kern);
}
EXPORT_SYMBOL(vca_vringh_iov_push_kern);
 ...
 */
void vca_vringh_abandon_kern(struct vringh *vrh, unsigned int num)
{
	/* We only update vring_avail_event(vr) when we want to be notified,
	 * so we haven't changed that yet. */
	vrh->last_avail_idx -= num;
}
EXPORT_SYMBOL(vca_vringh_abandon_kern);
 ...
 * to this function.
 */
int vca_vringh_complete_kern(struct vringh *vrh, u16 head, u32 len)
{
	struct vring_used_elem used;
...
EXPORT_SYMBOL(vca_vringh_complete_kern);
 ...
 * now more buffers available in the vring.
 */
bool vca_vringh_notify_enable_kern(struct vringh *vrh)
{
	return __vringh_notify_enable(vrh, getu16_kern, putu16_kern);
}
EXPORT_SYMBOL(vca_vringh_notify_enable_kern);
 ...
 * we're going to sleep.
 */
void vca_vringh_notify_disable_kern(struct vringh *vrh)
{
	__vringh_notify_disable(vrh, putu16_kern);
}
EXPORT_SYMBOL(vca_vringh_notify_disable_kern);
 ...
 * Returns -errno or 0 if we don't need to tell the other side, 1 if we do.
 */
int vca_vringh_need_notify_kern(struct vringh *vrh)
{
	return __vringh_need_notify(vrh, getu16_kern);
}
EXPORT_SYMBOL(vca_vringh_need_notify_kern);
