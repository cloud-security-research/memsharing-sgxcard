...
static void plx_dma_cleanup(struct plx_dma_chan *ch)
{
	...
	for (last_tail = ch->last_tail; last_tail != cached_head;) {
		...
		if (ACCESS_ONCE(ch->desc_ring[last_tail].dw0) & PLX_DESC_VALID) {
			/*
...
static int plx_dma_chan_setup(struct plx_dma_chan *ch)
{
	struct plx_dma_device *plx_dma_dev = to_plx_dma_dev(ch);
...
static int plx_dma_chan_init(struct plx_dma_chan *ch)
{
	...
	if (rc)
		goto ring_error;
	rc = plx_dma_chan_setup(ch);
	if (rc)
		goto chan_error;
	plx_debug_init();
	return rc;
chan_error:
	plx_dma_free_desc_ring(ch);
ring_error:
	dev_err(dev, "%s ret %d\n", __func__, rc);
	return rc;
}
...
int plx_dma_ring_count(u32 head, u32 tail)
{
	int count;
