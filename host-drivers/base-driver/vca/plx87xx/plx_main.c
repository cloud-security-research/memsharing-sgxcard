...
static int plx_map_resources(struct plx_device *xdev, struct pci_dev *pdev)
{
	...
	dev_info(&pdev->dev, "somnath: aper pa 0x%llx len 0x%llx va %p\n",
		     ...
		     const struct pci_device_id *ent)
{
	...
	g_vca_xdev[xdev->card_id][plx_identify_cpu_id(xdev)] = xdev;
	printk("Somnath *** g_vca_xdev[%d][%d] is %p\n",xdev->card_id,plx_identify_cpu_id(xdev),xdev);
	goto exit;
