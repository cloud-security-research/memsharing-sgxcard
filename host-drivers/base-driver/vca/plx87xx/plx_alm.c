...
int plx_alm_init(struct plx_alm* mngr, 
		struct pci_dev *pdev, int num_ntbs, u64 aper_len)
{
	int rc = 0;

	mngr->segments_num = 256 / num_ntbs;
		...
		u32 *out_segment_id, u32 * out_segments_num)
{
	...
	if (size > mngr->segment_size * mngr->segments_num) {
		dev_err(&pdev->dev, "allocation to big for translation\n");
		rc = -ENOMEM;
		goto finish;
	}
