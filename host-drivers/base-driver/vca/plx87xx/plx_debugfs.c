...
static int
plx_msi_irq_info_debug_release(struct inode *inode, struct file *file)
{
...
static const struct file_operations msi_irq_info_ops = {
	...
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = plx_msi_irq_info_debug_release
};
...
static int plx_memory_read_debug_release(struct inode *inode,
 struct file *file)
{
	return single_release(inode, file);
}
...
static const struct file_operations plx_memory_read_ops = {
	...
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = plx_memory_read_debug_release
};
...
static int
plx_memory_write_debug_release(struct inode *inode, struct file *file)
{
	return single_release(inode, file);
}
...
static const struct file_operations plx_memory_write_ops = {
	...
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = plx_memory_write_debug_release
};
...
static int plx_spad_show(struct seq_file *s, void *pos)
{
	...
	for (i = 0; i < 8; i++) {
		reg = plx_read_spad(xdev, i);
		seq_printf(s, "read spad[%d]=0x%x\n", i, reg);
		plx_write_spad(xdev, i, base + i);
		seq_printf(s, "wrote spad[%d]=0x%x\n", i, base + i);
		reg = plx_read_spad(xdev, i);
		seq_printf(s, "read spad[%d]=0x%x\n", i, reg);
	}
	return 0;
...
static int plx_spad_debug_release(struct inode *inode, struct file *file)
{
	return single_release(inode, file);
}
...
static const struct file_operations spad_ops = {
	...
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = plx_spad_debug_release
};
...
static int plx_alm_debug_release(struct inode *inode, struct file *file)
{
	return single_release(inode, file);
}
...
static int plx_intr_req_debug_release(struct inode *inode, struct file *file)
{
	return single_release(inode, file);
}
...
static const struct file_operations intr_req_ops = {
	...
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = plx_intr_req_debug_release
};
...
static int plx_intr_free_debug_release(struct inode *inode, struct file *file)
{
	return single_release(inode, file);
}
...
static const struct file_operations intr_free_ops = {
	...
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = plx_intr_free_debug_release
};
...
static int plx_intr_send_debug_release(struct inode *inode, struct file *file)
{
	return single_release(inode, file);
}
...
static const struct file_operations intr_send_ops = {
	...
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = plx_intr_send_debug_release
};
...
static int plx_dump_regs_complete_debug_release( struct inode *inode, struct file *file)
{
	return single_release(inode, file);
}
...
static const struct file_operations dump_regs_complete_ops = {
	...
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = plx_dump_regs_complete_debug_release
};
...
static int plx_dump_regs_single_debug_release( struct inode *inode, struct file *file)
{
	return single_release(inode, file);
}
...
static const struct file_operations dump_regs_single_ops = {
	...
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = plx_dump_regs_single_debug_release
};
