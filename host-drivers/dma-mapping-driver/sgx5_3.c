...
static int __init sgx5_10_init (void)
{
        int error = 0;

	somnath_xdev = (struct plx_device *)vca_get_xdev_pointer(1,0); 		
	//somnath_xdev = (struct plx_device *)0xffff88189a38a800ULL; 		
//	printk("somnath: vca get xdev pointer %p\n",vca_get_xdev_pointer);
	printk("somnath 1 0 : xdev %p aper.va %p aper.pa 0x%lx\n",somnath_xdev, somnath_xdev->aper.va, somnath_xdev->aper.pa);

        sgx5_kobject = kobject_create_and_add("sgx5_mapper_3",kernel_kobj);
        if(!sgx5_kobject)
                return -ENOMEM;

        error = sysfs_create_file(sgx5_kobject, &map0_attribute.attr);
        if (error) {
                pr_debug("somnath: failed to create the map 0 file in /sys/kernel/sgx5_mapper_10 \n");
        }


        error = sysfs_create_file(sgx5_kobject, &map1_attribute.attr);
        if (error) {
                pr_debug("somnath: failed to create the map 1 file in /sys/kernel/sgx5_mapper_10 \n");
        }

        return error;
}

static void __exit sgx5_10_exit (void)
{
        pr_debug ("somnath: Module 00 un initialized successfully \n");
        kobject_put(sgx5_kobject);
}


module_init(sgx5_10_init);
module_exit(sgx5_10_exit);
MODULE_LICENSE("GPL");
