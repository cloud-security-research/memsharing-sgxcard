...
static int __init sgx5_01_init (void)
{
        int error = 0;

	somnath_xdev = (struct plx_device *)vca_get_xdev_pointer(0,1); 		
	//somnath_xdev = (struct plx_device *)0xffff88189a38a800ULL; 		
//	printk("somnath: vca get xdev pointer %p\n",vca_get_xdev_pointer);
	printk("somnath 0 1 : xdev %p aper.va %p aper.pa 0x%lx\n",somnath_xdev, somnath_xdev->aper.va, somnath_xdev->aper.pa);

        sgx5_kobject = kobject_create_and_add("sgx5_mapper_1",kernel_kobj);
        if(!sgx5_kobject)
                return -ENOMEM;

        error = sysfs_create_file(sgx5_kobject, &map0_attribute.attr);
        if (error) {
                pr_debug("somnath: failed to create the map 0 file in /sys/kernel/sgx5_mapper_01 \n");
        }


        error = sysfs_create_file(sgx5_kobject, &map1_attribute.attr);
        if (error) {
                pr_debug("somnath: failed to create the map 1 file in /sys/kernel/sgx5_mapper_01 \n");
        }

        return error;
}

static void __exit sgx5_01_exit (void)
{
        pr_debug ("somnath: Module 01 un initialized successfully \n");
        kobject_put(sgx5_kobject);
}


module_init(sgx5_01_init);
module_exit(sgx5_01_exit);
MODULE_LICENSE("GPL");
